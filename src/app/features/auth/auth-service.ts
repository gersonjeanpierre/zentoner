...
export class AuthService {
  ...
  private edgeFunctionUrl = `${environment.SUPABASE_URL}/functions/v1/user-create-with-audit`;
  ...
  async registerEmployeeSecurely(payload: EdgeFunctionPayload): Promise<EdgeFunctionResponse> {
    ...
    const { data: { session }, error: sessionError } = await this.authSupabaseClient.auth.getSession();
    ...
    try {
      ...
      const serverError = httpError.error || { error: 'Error de red o Edge Function no disponible.' };
